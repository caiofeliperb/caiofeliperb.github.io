...
const NarrativeMap = () => {
    ...
    useEffect(() => {
        ...
        // Setup canvas particles for Step 0 was removed upon user request
        ...
        stepsRef.current.forEach((step, i) => {
            ScrollTrigger.create({
                ...
                onEnter: () => animateStep(i + 1, true), // Offset index since array starts at index 1 for scrolling
                onEnterBack: () => animateStep(i + 1, false),
            });
        });

        // Hide the map initially so the textual box appears first
    ...
    const animateStep = (index, isForward) => {
        // SVG MAP FADES IN DELAYED ON STEP 0 TO LET TEXT BOX APPEAR FIRST
        if (index === 0 && isForward) {
            gsap.to(svgContainerRef.current, { opacity: 1, duration: 1, delay: 0.3, overwrite: true });
        } else {
            gsap.to(svgContainerRef.current, { opacity: 1, duration: 0.5, overwrite: true });
        }
        ...
        if (index === 0) {
            ...
            gsap.to(labelRNRef.current, { opacity: 1, scale: 1, duration: 0.8, delay: 0.5, ease: "back.out(1.5)", overwrite: true });
            ...
            gsap.to(labelNERef.current, { opacity: 1, scale: 1, duration: 0.8, delay: 0.5, ease: "back.out(1.5)", overwrite: true });
